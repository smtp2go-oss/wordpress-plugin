 ...
 */
namespace SMTP2GOWPPlugin\Humbug\PhpScoper\Console\Command;

use SMTP2GOWPPlugin\Humbug\PhpScoper\Autoload\ScoperAutoloadGenerator;
use SMTP2GOWPPlugin\Humbug\PhpScoper\Configuration;
use SMTP2GOWPPlugin\Humbug\PhpScoper\Console\ScoperLogger;
use SMTP2GOWPPlugin\Humbug\PhpScoper\Scoper;
use SMTP2GOWPPlugin\Humbug\PhpScoper\Scoper\ConfigurableScoper;
use SMTP2GOWPPlugin\Humbug\PhpScoper\Throwable\Exception\ParsingException;
use SMTP2GOWPPlugin\Humbug\PhpScoper\Whitelist;
use SMTP2GOWPPlugin\Symfony\Component\Console\Exception\RuntimeException;
...
use SMTP2GOWPPlugin\Symfony\Component\Console\Input\StringInput;
use SMTP2GOWPPlugin\Symfony\Component\Console\Output\OutputInterface;
use SMTP2GOWPPlugin\Symfony\Component\Console\Style\OutputStyle;
...
use function getcwd;
use function SMTP2GOWPPlugin\Humbug\PhpScoper\get_common_path;
use function is_dir;
...
final class AddPrefixCommand extends BaseCommand
{
    ...
    {
        parent::configure();
        $this->setName('add-prefix')->setDescription('Goes through all the PHP files found in the given paths to apply the given prefix to namespaces & FQNs.')->addArgument(self::PATH_ARG, InputArgument::IS_ARRAY, 'The path(s) to process.')->addOption(self::PREFIX_OPT, 'p', InputOption::VALUE_REQUIRED, 'The namespace prefix to add.')->addOption(self::OUTPUT_DIR_OPT, 'o', InputOption::VALUE_REQUIRED, 'The output directory in which the prefixed code will be dumped.', 'build')->addOption(self::FORCE_OPT, 'f', InputOption::VALUE_NONE, 'Deletes any existing content in the output directory without any warning.')->addOption(self::STOP_ON_FAILURE_OPT, 's', InputOption::VALUE_NONE, 'Stops on failure.')->addOption(self::CONFIG_FILE_OPT, 'c', InputOption::VALUE_REQUIRED, sprintf('Configuration file. Will use "%s" if found by default.', self::CONFIG_FILE_DEFAULT))->addOption(self::NO_CONFIG_OPT, null, InputOption::VALUE_NONE, 'Do not look for a configuration file.');
    }
    /**
     * @inheritdoc
     */
    protected function execute(InputInterface $input, OutputInterface $output) : int
    {
        ...
        $logger = new ScoperLogger($this->getApplication(), $io);
     ...
     * @var callable[]
     */
    private function scopeFiles(string $prefix, array $filesWithContents, string $output, array $patchers, Whitelist $whitelist, bool $stopOnFailure, ScoperLogger $logger) : void
    {
     ...
     */
    private function scopeFile(string $inputFilePath, string $inputContents, string $outputFilePath, string $prefix, array $patchers, Whitelist $whitelist, bool $stopOnFailure, ScoperLogger $logger) : void
    {
        try {
    ...
    private function validateOutputDir(InputInterface $input, OutputStyle $io) : void
    {
        ...
        if (\false === is_writable($outputDir)) {
            throw new RuntimeException(sprintf('Expected "<comment>%s</comment>" to be writeable.', $outputDir));
        }
    ...
    private function retrieveConfig(InputInterface $input, OutputInterface $output, OutputStyle $io) : Configuration
    {
        ...
        if (null === $configFile) {
            ...
            if (\false === $this->init && \false === file_exists($configFile)) {
                ...
                $io->writeln(sprintf('Config file "<comment>%s</comment>" not found. Skipping.', $configFile), OutputInterface::VERBOSITY_DEBUG);
        ...
        if (null === $configFile) {
            ...
            throw new RuntimeException(sprintf('Could not find the configuration file "%s".', $configFile));
        } else {
            $io->writeln(sprintf('Using the configuration file "%s".', $configFile), OutputInterface::VERBOSITY_DEBUG);
        }
