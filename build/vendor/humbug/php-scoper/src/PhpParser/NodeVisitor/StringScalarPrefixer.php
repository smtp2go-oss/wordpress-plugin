...
use SMTP2GOWPPlugin\PhpParser\Node\Expr\ArrayItem;
use SMTP2GOWPPlugin\PhpParser\Node\Expr\Assign;
...
use SMTP2GOWPPlugin\PhpParser\Node\Stmt\PropertyProperty;
use SMTP2GOWPPlugin\PhpParser\Node\Stmt\Return_;
...
final class StringScalarPrefixer extends NodeVisitorAbstract
{
    ...
    {
        if (\false === (ParentNodeAppender::hasParent($string) && is_string($string->value)) || 1 !== preg_match('/^((\\\\)?[\\p{L}_\\d]+)$|((\\\\)?(?:[\\p{L}_\\d]+\\\\+)+[\\p{L}_\\d]+)$/u', $string->value)) {
            return $string;
        }
        ...
        if (\false === ($parentNode instanceof Assign || $parentNode instanceof Param || $parentNode instanceof Const_ || $parentNode instanceof PropertyProperty || $parentNode instanceof Return_)) {
            return $string;
        }
    ...
    private function prefixArrayItemString(String_ $string, ArrayItem $parentNode, string $normalizedValue) : String_
    {
        ...
        $parentNode = ParentNodeAppender::getParent($parentNode);
        if (\false === $parentNode instanceof Arg || null === ($functionNode = ParentNodeAppender::findParent($parentNode))) {
            // If belongs to the global namespace then we cannot differentiate the value from a symbol and a regular string
            return $this->belongsToTheGlobalNamespace($string) ? $string : $this->createPrefixedString($string);
        }
    ...
    private function isConstantNode(String_ $node) : bool
    {
        ...
        if (\false === $argParent->name instanceof Name || 'define' !== (string) $argParent->name && 'defined' !== (string) $argParent->name) {
            return \false;
        }
