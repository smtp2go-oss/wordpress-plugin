...
use SMTP2GOWPPlugin\Humbug\PhpScoper\PhpParser\Node\ClassAliasFuncCall;
...
use SMTP2GOWPPlugin\PhpParser\Node\Stmt\Expression;
use SMTP2GOWPPlugin\PhpParser\Node\Stmt\Interface_;
use SMTP2GOWPPlugin\PhpParser\Node\Stmt\Namespace_;
...
final class ClassAliasStmtAppender extends NodeVisitorAbstract
{
    ...
    private function createNamespaceStmts(array $stmts, Stmt $stmt) : array
    {
        ...
        if (\false === $originalName instanceof FullyQualified || $this->whitelist->belongsToWhitelistedNamespace((string) $originalName) || \false === $this->whitelist->isSymbolWhitelisted((string) $originalName) && \false === $this->whitelist->isGlobalWhitelistedClass((string) $originalName)) {
            return $stmts;
        }
    ...
    {
        $call = new ClassAliasFuncCall(FullyQualifiedFactory::concat($this->prefix, $originalName), $originalName, $stmt->getAttributes());
