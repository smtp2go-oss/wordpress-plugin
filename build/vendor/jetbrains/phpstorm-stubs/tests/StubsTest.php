...
namespace SMTP2GOWPPlugin\StubTests;
...
use SMTP2GOWPPlugin\phpDocumentor\Reflection\DocBlock\Tags\Reference\Url;
...
use SMTP2GOWPPlugin\StubTests\Model\BasePHPClass;
use SMTP2GOWPPlugin\StubTests\Model\BasePHPElement;
use SMTP2GOWPPlugin\StubTests\Model\PHPClass;
use SMTP2GOWPPlugin\StubTests\Model\PHPConst;
use SMTP2GOWPPlugin\StubTests\Model\PHPDocElement;
...
use SMTP2GOWPPlugin\StubTests\Model\PHPParameter;
use SMTP2GOWPPlugin\StubTests\Model\StubProblemType;
use SMTP2GOWPPlugin\StubTests\Model\Tags\RemovedTag;
use SMTP2GOWPPlugin\StubTests\Parsers\Utils;
use SMTP2GOWPPlugin\StubTests\TestData\Providers\PhpStormStubsSingleton;
...
class StubsTest extends TestCase
{
    ...
    public function testConstants(PHPConst $constant) : void
    {
        ...
        static::assertArrayHasKey($constantName, $stubConstants, "Missing constant: const {$constantName} = {$constantValue}\n");
    }
    /**
     * @dataProvider \StubTests\TestData\Providers\ReflectionTestDataProviders::constantProvider
     */
    public function testConstantsValues(PHPConst $constant) : void
    {
        ...
        static::assertEquals($constantValue, $stubConstants[$constantName]->value, "Constant value mismatch: const {$constantName} \n\n            Expected value: {$constantValue} but was {$stubConstants[$constantName]->value}");
    }
    /**
    ...
    public function testFunctions(PHPFunction $function) : void
    {
        ...
        if (!$function->hasMutedProblem(StubProblemType::FUNCTION_IS_DEPRECATED)) {
            static::assertFalse($function->is_deprecated && $phpstormFunction->is_deprecated !== \true, "Function {$functionName} is not deprecated in stubs");
        }
        if (!$function->hasMutedProblem(StubProblemType::FUNCTION_PARAMETER_MISMATCH)) {
            static::assertSameSize($function->parameters, $phpstormFunction->parameters, "Parameter number mismatch for function {$functionName}. \n                Expected: " . self::getParameterRepresentation($function));
            if ($phpstormFunction->stubBelongsToCore) {
                foreach ($function->parameters as $parameter) {
                    if (!$parameter->hasMutedProblem(StubProblemType::PARAMETER_NAME_MISMATCH)) {
                        self::assertNotEmpty(array_filter($phpstormFunction->parameters, fn(PHPParameter $stubParameter) => $stubParameter->name === $parameter->name), "Function {$functionName} has signature {$functionName}(" . $this->printParameters($function->parameters) . ')' . " but stub function has signature {$functionName}(" . $this->printParameters($phpstormFunction->parameters) . ")");
    ...
    public function testClasses(PHPClass $class) : void
    {
        ...
        if (!$class->hasMutedProblem(StubProblemType::WRONG_PARENT)) {
            static::assertEquals($class->parentClass, $stubClass->parentClass, "Class {$className} should extend {$class->parentClass}");
        }
        foreach ($class->constants as $constant) {
            if (!$constant->hasMutedProblem(StubProblemType::STUB_IS_MISSED)) {
                static::assertArrayHasKey($constant->name, $stubClass->constants, "Missing constant {$className}::{$constant->name}");
            }
        }
        foreach ($class->methods as $method) {
            ...
            if (!$method->hasMutedProblem(StubProblemType::STUB_IS_MISSED)) {
                static::assertArrayHasKey($methodName, $stubClass->methods, "Missing method {$className}::{$methodName}({$params}){}");
                $stubMethod = $stubClass->methods[$methodName];
                if (!$method->hasMutedProblem(StubProblemType::FUNCTION_IS_FINAL)) {
                    static::assertEquals($method->is_final, $stubMethod->is_final, "Method {$className}::{$methodName} final modifier is incorrect");
                }
                if (!$method->hasMutedProblem(StubProblemType::FUNCTION_IS_STATIC)) {
                    static::assertEquals($method->is_static, $stubMethod->is_static, "Method {$className}::{$methodName} static modifier is incorrect");
                }
                if (!$method->hasMutedProblem(StubProblemType::FUNCTION_ACCESS)) {
                    static::assertEquals($method->access, $stubMethod->access, "Method {$className}::{$methodName} access modifier is incorrect");
                }
                if (!$method->hasMutedProblem(StubProblemType::FUNCTION_PARAMETER_MISMATCH)) {
                    static::assertSameSize($method->parameters, $stubMethod->parameters, "Parameter number mismatch for method {$className}::{$methodName}. \n                        Expected: " . self::getParameterRepresentation($method));
                    foreach ($method->parameters as $parameter) {
                        self::assertNotEmpty(array_filter($stubMethod->parameters, fn(PHPParameter $stubParameter) => $stubParameter->name === $parameter->name), "Function {$className}::{$methodName} has signature {$methodName}(" . $this->printParameters($method->parameters) . ')' . " but stub function has signature {$methodName}(" . $this->printParameters($stubMethod->parameters) . ")");
        ...
        foreach ($class->properties as $property) {
            ...
            if (!$property->hasMutedProblem(StubProblemType::STUB_IS_MISSED)) {
                static::assertArrayHasKey($propertyName, $stubClass->properties, "Missing property {$className}::{$property->access} {$property->type} \${$propertyName}");
                $stubProperty = $stubClass->properties[$propertyName];
                if (!$property->hasMutedProblem(StubProblemType::PROPERTY_IS_STATIC)) {
                    static::assertEquals($property->is_static, $stubProperty->is_static, "Property {$className}::{$propertyName} static modifier is incorrect");
                }
                if (!$property->hasMutedProblem(StubProblemType::PROPERTY_ACCESS)) {
                    static::assertEquals($property->access, $stubProperty->access, "Property {$className}::{$propertyName} access modifier is incorrect");
                }
                if (!$property->hasMutedProblem(StubProblemType::PROPERTY_TYPE) && !empty($property->type)) {
                    static::assertEquals($property->type, $stubProperty->type, "Property type doesn't match for property {$className}::{$propertyName}");
                }
            }
        }
    }
    ...
    public function testInterfaces(PHPInterface $interface) : void
    {
        ...
        static::assertArrayHasKey($interfaceName, $stubInterfaces, "Missing interface {$interfaceName}: interface {$interfaceName} {}");
        ...
        if (!$interface->hasMutedProblem(StubProblemType::WRONG_PARENT)) {
            foreach ($interface->parentInterfaces as $parentInterface) {
                static::assertContains($parentInterface, $stubInterface->parentInterfaces, "Missing parent interface {$parentInterface}");
            }
        }
        foreach ($interface->constants as $constant) {
            if (!$constant->hasMutedProblem(StubProblemType::STUB_IS_MISSED)) {
                static::assertArrayHasKey($constant->name, $stubInterface->constants, "Missing constant {$interfaceName}::{$constant->name}");
            }
        }
        foreach ($interface->methods as $method) {
            ...
            $methodName = $method->name;
            if (!$method->hasMutedProblem(StubProblemType::STUB_IS_MISSED)) {
                static::assertArrayHasKey($methodName, $stubInterface->methods, "Missing method {$interfaceName}::{$methodName}({$params}){}");
                $stubMethod = $stubInterface->methods[$methodName];
                if (!$method->hasMutedProblem(StubProblemType::FUNCTION_IS_FINAL)) {
                    static::assertEquals($method->is_final, $stubMethod->is_final, "Method {$interfaceName}::{$methodName} final modifier is incorrect");
                }
                if (!$method->hasMutedProblem(StubProblemType::FUNCTION_IS_STATIC)) {
                    static::assertEquals($method->is_static, $stubMethod->is_static, "Method {$interfaceName}::{$methodName} static modifier is incorrect");
                }
                if (!$method->hasMutedProblem(StubProblemType::FUNCTION_ACCESS)) {
                    static::assertEquals($method->access, $stubMethod->access, "Method {$interfaceName}::{$methodName} access modifier is incorrect");
                }
                if (!$method->hasMutedProblem(StubProblemType::FUNCTION_PARAMETER_MISMATCH)) {
                    static::assertSameSize($method->parameters, $stubMethod->parameters, "Parameter number mismatch for method {$interfaceName}::{$methodName}. \n                        Expected: " . self::getParameterRepresentation($method));
                }
            }
        }
    }
    ...
    private function checkLinks(BasePHPElement $element, string $elementName) : void
    {
        ...
        foreach ($element->links as $link) {
            if ($link instanceof Link) {
                static::assertStringStartsWith('https', $link->getLink(), "In {$elementName} @link doesn't start with https");
    ...
    private static function checkFunctionDoesNotHaveScalarTypeHints(int $sinceVersion, PHPFunction $function)
    {
        if ($sinceVersion < 7) {
            if (empty($function->parameters)) {
                self::assertTrue(\true, 'Parameters list empty');
            } else {
                foreach ($function->parameters as $parameter) {
                    if (!$parameter->hasMutedProblem(StubProblemType::PARAMETER_HAS_SCALAR_TYPEHINT)) {
                        self::assertFalse($parameter->type === 'int' || $parameter->type === 'float' || $parameter->type === 'string' || $parameter->type === 'bool', "Function '{$function->name}' with @since '{$sinceVersion}'  \n                has parameter '{$parameter->name}' with typehint '{$parameter->type}' but typehints available only since php 7");
                    } else {
            ...
            self::assertTrue(\true, "Function '{$function->name}' has since version > 7");
        }
    }
