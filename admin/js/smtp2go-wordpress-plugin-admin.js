  ...
  $(function () {
    ...
    $(".js-send-test-email-form").submit(function (e) {
      ...
      $.post(
        ...
        $(".js-send-test.spinner").removeClass("is-active");
        $(".smtp2go-js-failure").text(jqXHR.responseText).show();
        $(".smtp2go-js-success").hide();
      }),
        "jSON";
      return false;
    });
    $('.js-smtp2go_delete_api_key').on('click', function () {
      var r = confirm("Are you sure you want to remove the API key from the database?");
      if (r == true) {
        $.post(
          ajaxurl, //from html header
          {
